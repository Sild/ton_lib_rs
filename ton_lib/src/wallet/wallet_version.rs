...
impl WalletVersion {
    ...
    pub fn get_code(version: WalletVersion) -> Result<&'static TonCellRef, TonlibError> {
        TON_WALLET_CODE_BY_VERSION
            .get(&version)
            .ok_or_else(|| TonlibError::Custom(format!("No code found for {version:?}")))
    }

    pub fn code_by_version(ver: WalletVersion) -> Result<&'static TonCellRef, TonlibError> {
        TON_WALLET_CODE_BY_VERSION
            .get(&ver)
            .ok_or_else(|| TonlibError::Custom(format!("No code found for version: {ver:?}")))
    }
    ...
    pub fn version_by_code(code_hash: TonHash) -> Result<WalletVersion, TonlibError> {
        TON_WALLET_VERSION_BY_CODE
            .get(&code_hash)
            .copied()
            .ok_or_else(|| TonlibError::Custom(format!("No version found for code_hash: {code_hash}")))
    }
    ...
    pub fn build_ext_in_body(
        ...
        let res = match version {
            ...
            .to_cell(),
            _ => Err(TonlibError::Custom(format!("build_ext_in_body for {version:?} is unsupported"))),
        };
