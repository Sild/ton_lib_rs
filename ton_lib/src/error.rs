...
pub enum TLError {
    #[error("TLCoreError: {0}")]
    TLCoreError(#[from] TonlibError),
    #[error("TLCoreError: {0}")]
    TLCoreArcError(#[from] Arc<TonlibError>),
    // boc
...
impl From<TLError> for TonlibError {
    fn from(err: TLError) -> Self { TonlibError::Custom(err.to_string()) }
}
