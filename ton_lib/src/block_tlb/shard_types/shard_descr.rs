...
impl TLB for ShardDescr {
    ...
    fn read(parser: &mut CellParser) -> Result<Self, TonlibError> { Self::read_definition(parser) }

    fn write(&self, builder: &mut CellBuilder) -> Result<(), TonlibError> { self.write_definition(builder) }
}
