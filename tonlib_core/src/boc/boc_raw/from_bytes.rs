...
impl BOCRaw {
    ...
    pub fn from_bytes(serial: &[u8]) -> Result<BOCRaw, TonlibError> {
        ...
        let (has_idx, has_crc32c, _has_cache_bits, size) = {
            ...
            if size > 4 {
                return Err(TonlibError::Custom(format!("Invalid BoC header: size({size}) <= 4")));
            }
        ...
        if off_bytes > 8 {
            return Err(TonlibError::Custom(format!("Invalid BoC header: off_bytes({off_bytes}) <= 8")));
        }
        //cells:(##(size * 8))
        ...
        if roots_cnt < 1 {
            return Err(TonlibError::Custom(format!("Invalid BoC header: roots({roots_cnt}) >= 1")));
        }
        //   absent:(##(size * 8)) { roots + absent <= cells }
        let absent = read_var_size(&mut reader, size)?;
        if roots_cnt + absent > cells_cnt {
            return Err(TonlibError::Custom(format!(
...
fn read_cell(reader: &mut ByteReader<Cursor<&[u8]>, BigEndian>, size: u8) -> Result<CellRaw, TonlibError> {
    ...
    let cell_type = match is_exotic {
        true => {
            if data.is_empty() {
                return Err(TonlibError::Custom("Exotic cell must have at least 1 byte".to_string()));
            }
